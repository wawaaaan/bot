...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `hey kak @${num.split('@')[0]} wans Titip Gorengan Yah Btw Gausa Kembali Lagi... Kembali = kick 😋`
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '[TUNGGU] wans sedang Memperoses🍥Orang sabar Di Sayang tuhan!',
				asik: '[TUNGGU] wans sedang Memperoses🍥Orang sabar Di Sayang tuhan!',
				...
				only: {
					...
					mod: '[❗] PERINTAH INI KHUSUS USER *MOD wans BOT*',
					...
					userB: `──「 DAFTAR 」──\nHalo Kak !\nMaaf Kak Kamu belum Terdaftar Sebagai Teman wans Daftar Untuk Berteman Dengan Ici bot dengam cara, \n\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar wans|17\n\n──「 *wans BOT* 」──`,
			...
			const ownerNumber = ["6285852353712@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"6285852353712@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["6285852353712@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["6285852353712@s.whatsapp.net"]//ubah nomor lo
			const premium = ["6285852353712@s.whatsapp.net","6282286737721@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isUser) return reply(mess.only.userB)
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT wans ✨* 」\n*│+wa.me/6281368646011*╰──────*「 *wans* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  wans BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/PcQ6tsB/79ac87b9358c.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/6281368646011*╰──────*「 * wans* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  wans BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				 data = fs.readFileSync('./src/wansjokes.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                client.sendMessage(hasil, image, mek, '\`\`\`wansJOKES\`\`\`')
				break
					case 'kata':
 ...
 if (!isOwner) return reply(mess.only.ownerB)
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘  wans BOT 〙', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
	...
	.then(() => {client.sendMessage(from, options, text)})
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner wans!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin wans!')
				   }
                   break
				case 'wait':
				if (isBanned) return reply(mess.only.benned)    
